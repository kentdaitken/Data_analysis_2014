...
path = '/Users/sdsdf/Documents/Plain Language/'
print('\nFILES:\n')
...
banned = {'agenda', 'advancing', 'collaborate', 'combating', 'commit', 'pledge', 'countering',
          'deploy', 'dialogue', 'disincentivize', 'incentivize', 'empower', 'facilitate', 'focusing', 'empower',
          'foster', 'impact', 'initiate', 'innovative', 'key', 'land', 'leverage', 'overarching', 'progress',
          'promote', 'robust', 'simple', 'simply', 'streamline', 'strengthen', 'strengthening', 'tackling',
          'thought leader', 'touchpoint', 'transform', 'transforming', 'user testing', 'utilize', 'drive',
          'drive out', 'going foward', 'in order to', 'one-stop shop'}
...
def text_analysis(text):
    ...
    options = ['Find collocations', 'Find Concordance', 'Run a frequency distribution', 'Display Hapaxes',
               ...
               'Search for 3 word phrases', 'Banned concordance', 'Positive concordance']
...
def banned_concordance(text):
    print("Printing concordance by banned words")

    save_file = open(path + '{} banned.txt'.format(ftarget), 'w')
    save_file.write("Printing concordance by Banned {}".format(time.strftime("%d/%m/%Y")))
    save_file.write('\n')
    #for t in tools:
    for w in banned:
        print(w.upper())
